...
public class BorderLayout implements LayoutManager2, java.io.Serializable {
	...
	public void layoutContainer(Container parent) {
		if (!created) {
			...
			table = org.teavm.jso.browser.Window.current().getDocument().createElement("table");
			...
			for (int j = 0; j < 3; j++) {
				HTMLElement row = org.teavm.jso.browser.Window.current().getDocument().createElement("tr");
				table.appendChild(row);
				if(j != 1) {
					row.getStyle().setProperty("height", "0px");
				} else {
					row.getStyle().setProperty("height", "100%");
				}
				for (int i = 0; i < 3; i++) {
					if (j == 1 || i == 1) {
						HTMLElement col = org.teavm.jso.browser.Window.current().getDocument().createElement("td");
						col.getStyle().setProperty("padding", "0px");
						row.appendChild(col);
						if (i == 1) {
							col.getStyle().setProperty("width", "100%");
							if (j != 1) {
								col.setAttribute("colSpan", "3");
							}
						} else {
							col.getStyle().setProperty("width", "0px");
						}
					}
				}
			}
		...
		for (int j = 0; j < 3; j++) {
			for (int i = 0; i < 3; i++) {
				if (pos++ == position) {
					...
					HTMLElement col = (HTMLElement) row.getChildNodes().get(j == 1 ? i : 0);
