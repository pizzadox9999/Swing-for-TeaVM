...
public class Font implements java.io.Serializable {
	...
	public static void setElementSizeForString(HTMLElement element, String text) {
		
		String font = element.getStyle().getPropertyValue("font");

		HTMLCanvasElement canvas = (HTMLCanvasElement) Window.current().getDocument().createElement("canvas");
		CanvasRenderingContext2D context = (CanvasRenderingContext2D) canvas.getContext("2d");
		context.setFont(font);
		
		element.getStyle().setProperty("width", String.valueOf(context.measureText(text).getWidth()+10));
	}
