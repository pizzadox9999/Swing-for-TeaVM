...
import org.tgui.awt.event.KeyEvent;
import org.tgui.beans.PropertyChangeEvent;
import org.tgui.beans.PropertyChangeListener;
import org.tgui.swing.event.ListDataEvent;
import org.tgui.swing.event.ListDataListener;
import org.tgui.swing.event.PopupMenuEvent;
import org.tgui.swing.event.PopupMenuListener;
	...
	public void initHTML() {
		...
		getHTMLElement().addEventListener("change", new EventListener<Event>() {
			public void handleEvent(Event evt) {
				...
				fireItemStateChanged(new ItemEvent(self, 0, getItemAt(lastSelected), ItemEvent.DESELECTED));
				fireItemStateChanged(new ItemEvent(self, 0, getItemAt(i), ItemEvent.SELECTED));
				lastSelected = i;
			}
		});
		...
		for (int i = 0; i < getItemCount(); i++) {
			...
			// TODO: use renderer
			option.setInnerHTML(getItemAt(i).toString());
			option.setValue(getItemAt(i).toString());
