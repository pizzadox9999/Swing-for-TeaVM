...
public class JTextField extends JTextComponent implements SwingConstants {
	...
	public void createHTML() {
		...
		getHTMLElement().getStyle().setPropertyValue("backgroundColor", Color.WHITE.toHTML());
		getHTMLElement().getStyle().setPropertyValue("border", "2px groove " + new Color(180, 185, 196).toHTML());
		getHTMLElement().getStyle().setPropertyValue("padding", "0px");
		getHTMLElement().getStyle().setPropertyValue("boxSizing", "border-box");
	}

	@Override
	public void initHTML() {
		super.initHTML();
		getHTMLElement().setValue(text);
		getHTMLElement().setReadOnly(!editable);
		initActionListeners();
	}
