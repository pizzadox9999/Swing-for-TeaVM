...
import org.tgui.swing.event.ChangeEvent;
import org.tgui.swing.event.ChangeListener;
...
public class JSlider extends JComponent implements SwingConstants {
	...
	public void createHTML() {
		...
		getHTMLElement().getStyle().setPropertyValue("margin", "0px");
		getHTMLElement().getStyle().setPropertyValue("padding", "0px");
		getHTMLElement().setType("range");
		getHTMLElement().addEventListener("change", new EventListener<Event>() {
			public void handleEvent(Event evt) {
				setValue(Integer.parseInt(getHTMLElement().getValue()));
			}
		});
		getHTMLElement().addEventListener("input", new EventListener<Event>() {
			public void handleEvent(Event evt) {
				setValue(Integer.parseInt(getHTMLElement().getValue()));	
			}
		});
	...
	public void initHTML() {
		super.initHTML();
		getHTMLElement().setAttribute("min", "" + getMinimum());
		getHTMLElement().setAttribute("max", "" + getMaximum());
		getHTMLElement().setAttribute("step", "" + 1);
		getHTMLElement().setAttribute("value", "" + getValue());
		try {
	...
	public void setValue(int n) {
		...
		if (Integer.parseInt(getHTMLElement().getValue()) != getValue()) {
			getHTMLElement().setValue(String.valueOf(getValue()));
		}
	}
