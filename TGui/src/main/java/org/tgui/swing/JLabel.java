...
public class JLabel extends JComponent implements SwingConstants {
	...
	public void initHTML() {
		super.initHTML();
		while (getHTMLElement().getFirstChild() != null) {
			getHTMLElement().removeChild(getHTMLElement().getFirstChild());
		}
		if (defaultIcon != null) {
			htmlImageElement = defaultIcon.getInternalHTMLImageElement();
		}
		htmlLabelElement = Window.current().getDocument().createElement("label");
		if (defaultIcon != null) {
			htmlElement.appendChild(htmlImageElement);
		}
		htmlLabelElement.setInnerHTML(getText());

		htmlElement.getStyle().setPropertyValue("display", "table");
		htmlElement.getStyle().setPropertyValue("tableLayout", "fixed");
		htmlElement.getStyle().setPropertyValue("textAlign", "center");
		htmlLabelElement.getStyle().setPropertyValue("display", text == null || text.length() == 0 ? "none" : "table-cell");
		htmlLabelElement.getStyle().setPropertyValue("verticalAlign", "middle");
		htmlLabelElement.getStyle().setPropertyValue("textOverflow", "ellipsis");
		htmlLabelElement.getStyle().setPropertyValue("overflow", "hidden");
		htmlLabelElement.getStyle().setPropertyValue("whiteSpace", "nowrap");
		htmlLabelElement.getStyle().setPropertyValue("font", Font.decode(null).toHTML());
	...
	public void setText(String text) {
		...
		if (htmlLabelElement != null) {
			htmlLabelElement.setInnerHTML(text);
			htmlLabelElement.getStyle().setPropertyValue("display", text == null || text.length() == 0 ? "none" : "table-cell");
		}
	}
